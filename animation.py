...
img1 = cv2.imread('gridworld.jpg',0)

ret,world = cv2.threshold(img1,127,255,cv2.THRESH_BINARY)
height, width = img1.shape
showImg = copy.deepcopy(world)
...
def defineGoals_start():
    ...
    for i in range(X_min,X_min+13):
        startStates.append((i,Y_min+0))
        startStates.append((i,Y_min+1))
    
    for j in range(X_max-5,X_max):
        goalStates.append((j,Y_min+30))
        goalStates.append((j,Y_min+40))
   
    cv2.line(showImg, (X_min,Y_min+0), (X_min+13, Y_min+0), 1, 1)
    cv2.line(showImg, (X_min,Y_min+1), (X_min+13, Y_min+1), 1, 1)
    cv2.line(showImg, (X_max-5,Y_min+30), (X_max, Y_min+30), 1, 1)
    cv2.line(showImg, (X_max-5,Y_min+40), (X_max, Y_min+40), 1, 1)
...
def SARSA(numEpisodes):
    ...
    while(episodeCount != numEpisodes):
        ...
        for i in range(0,len(path_sarsa) - 1):
            cv2.line(showImg, path_sarsa[i][0], path_sarsa[i+1][0], 1, 1)
            cv2.imshow('window',showImg)
            cv2.waitKey(1)
            
        print "Episode complete in: ", stepCount, 'steps'
        episodeCount += 1
...
def EXP_SARSA(numEpisodes):
    ...
    while(episodeCount != numEpisodes):
        ...
        for i in range(0,len(path_expsarsa) - 1):
            cv2.line(showImg, path_expsarsa[i][0], path_expsarsa[i+1][0], 1, 1)
            cv2.imshow('window',showImg)
            cv2.waitKey(1)
...
def Q_learning(numEpisodes):
    ...
    while(episodeCount != numEpisodes):
        ...
        for i in range(0,len(path_q) - 1):
            cv2.line(showImg, path_q[i][0], path_q[i+1][0], 1, 1)
            cv2.imshow('window',showImg)
            cv2.waitKey(1)
...
if __name__ == '__main__':
    ...
    ######### THIS FILE FOR ANIMATION ONLY USES OTHER IMAGE ############
################ Uncomment algorithms to run one at time ######################
